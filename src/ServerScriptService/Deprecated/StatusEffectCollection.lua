...
--[Modules]
local ContainerFactory = require(ServerScriptService.Dependencies.ContainerFactory)
local StatusEffectLibrary = require(ServerScriptService.StatusEffectLibrary)

local StatusEffectCollection = setmetatable({}, { __index = ContainerFactory })
...
function StatusEffectCollection:ProcessEvent(triggerName, context)
	for _, statusEffect in pairs(self.items) do
