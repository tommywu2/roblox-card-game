...
function ClashHandler.ResolveSpeedDie(actorDie: SpeedDie)
	--Logging.
	print("Testing: " .. actorDie.owner.name .. " is resolving")
	
	local targetDie = actorDie:GetTargetSpeedDie()
	...
	print("Testing: Target: " .. target:GetName())
	
	while actorDie:GetAssignedPile():Count() > 0 do
		if targetDie:GetAssignedPile():Count() > 0 then
			local actorCard = actorDie:GetAssignedPile():GetAt(1)
			local targetCard = targetDie:GetAssignedPile():GetAt(1)
